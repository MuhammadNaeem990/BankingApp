...
import com.example.bankingapp.databinding.ViewholderItemsBinding

class ExpenseListAdapter(private val items:MutableList<ExpenseDomain>) :
	RecyclerView.Adapter<ExpenseListAdapter.Viewholder>() {
	
	
	class Viewholder(val binding:ViewholderItemsBinding):RecyclerView.ViewHolder
		(binding.root)
	...
	override fun onBindViewHolder(holder: ExpenseListAdapter.Viewholder, position: Int) {
		...
		val drawableResourceId = holder.itemView.resources.getIdentifier(item.pic,
